...
本文件是 Reasonix 平台接入"通用受控财务报表复核系统"的连接说明。
Reasonix 待通过同一符合性测试后才能标记"正式支持"；当前能力声明如下，
不伪造任何尚未验证的配置。
...
- **supports**: `local_command`
  - Reasonix 当前仅声明支持命令行入口接入本地核心。
  - MCP 支持能力未验证，不伪造声明。
- **defines**: `next_action_loop`、`resume_same_project`
  - Reasonix 计划实现"循环调用 project_next_action"的标准循环，
    并支持在同一项目中断后恢复继续执行。
- **never_allows**: `direct_database_write`
  - Reasonix 永远不得直接写入底层数据库，所有写入必须通过受控工具
    （`submit_review`、`finish_assignment` 等）经 AppService 进行。

## 平台特性

- Reasonix 通过命令行调用本地核心 `AppService`。
...
- **禁止** Reasonix 直接访问 `state/store.py` 的 SQLite 文件。
- **禁止** Reasonix 绕过 `AppService` 修改 `targets`、`assignments`、`issues` 表。
- **禁止** Reasonix 在 `submit_review` 之外修改复核结论。
